...
it was the grayish white of old porridge.";
		...
		//bool jeStejne = jmeno1.ToLower().Equals(jmeno2.ToLower());
		...
		if (string.Compare(min, blabol2) > 0)
		{
			min = blabol2;
		}

		if (string.Compare(min, blabol3) > 0)
		{
			min = blabol3;
		}
		...
		//string prvni = new SortedSet<string> { blabol1, blabol2, blabol3 }.Min;
		...
		int vykricnik = text.IndexOf("!", StringComparison.OrdinalIgnoreCase);

		//Console.WriteLine(vykricnik);

		string veta = text.Substring(1,vykricnik - 1);
		...
		for (int i = 0; i < slova.Length; i++)
		 { if (string.Equals("and", slova[i], StringComparison.OrdinalIgnoreCase))
				pocetAnd++;
		}
		//while ((index = text.IndexOf(" and ", index + 1, StringComparison.OrdinalIgnoreCase)) != -1) { pocetAnd++; }
